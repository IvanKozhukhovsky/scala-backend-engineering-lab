...
/** Cents per unit for a currency code, or None. Pure: no HTTP, no JDBC. */
def rateOf(code: String): Option[Int] =
  rates.get(code)

/** Injected values. Tests construct this; they do not own the process environment. */
final case class QuotesConfig(name: String, minQty: Int)
...
/** HTTP surface for `docs/capstone/quotes.openapi.yaml`. Building this does not bind a port. */
def quotesRoutes(config: QuotesConfig): HttpRoutes[IO] =
  HttpRoutes.of[IO] {
...
def quotesApp(config: QuotesConfig): HttpApp[IO] =
  quotesRoutes(config).orNotFound
...
object Day30 extends IOApp.Simple:
  val run: IO[Unit] =
    val app = quotesApp(QuotesConfig("quotes", 1))
